...
> 包名是 `git-commit-push`，注册的 **skill 名是 `commit-push`**（取自 `SKILL.md` 的 frontmatter，与仓库名不同）。
...
# 锁版本
pi install git:github.com/EvenToss/git-commit-push@v1.0.0
# 更新
...
pi 对 skills 目录是**递归发现**的，会自动找到 `skills/commit-push/SKILL.md`。
...
强制中文是默认。想改英文 / 加 scope / 换前缀集：直接编辑 `skills/commit-push/SKILL.md` 的【类型选择】与【提交信息格式】两节即可——纯文本，团队成员各自维护或随约定更新。

## 为什么这样设计（省 token 原理）

1. **渐进式披露**：平时只有 `name + description`（约 60 token）在系统提示里；只有真的要提交时才 `read` 完整 `SKILL.md`。
...
脚本会执行 `git add` / `git commit` / `git push`。首次使用前请 review `skills/commit-push/scripts/` 下的三个脚本。默认行为已尽量保守：
